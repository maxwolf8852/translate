...
func newTokenAcquirer(host string, client *http.Client) *tokenAcquirer {
...
func (a *tokenAcquirer) do(ctx context.Context, text string) (string, error) {
	err := a.update(ctx)
	if err != nil {
		return "", err
	}
...
func (a *tokenAcquirer) update(ctx context.Context) error {
	...
	req, err := http.NewRequestWithContext(ctx, http.MethodGet, a.host, http.NoBody)
	if err != nil {
		return err
	}
